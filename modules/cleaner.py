...
class FileCleaner:
    ...
    def clean_files(self, hard_clean=False, uniq_chars_map_file="", normalize_len_interval=[]):
        """
        ...
        for file_path in self.files_path:
            ...
            for en_char, fa_char in zip([str(i) for i in range(10)], ["۰", "۱", "۲", "۳", "۴", "۵", "۶", "۷", "۸", "۹"]):
                file = re.sub(en_char, fa_char, file, maxsize)
            file = re.sub(",", "،", file, maxsize, re.MULTILINE) # Replace English comma with Arabic comma
            file = re.sub("\?", "؟", file, maxsize, re.MULTILINE) # Replace English question mark to Arabic question mark
            file = re.sub(r"[ÙïřŠƘƙǘǙǠʙȘș¾ΘΙΠ٭%×Ϙ'~÷ϙϠЙРљµҘә@•\uf031\u202e\u202b\u2005\u00a0]", "", file, maxsize) # Remove unnecessary characters
            file = re.sub("[–_]", "-", file, maxsize) # Replace some character with HYPHEN-MINUS(-)
            file = re.sub("·", ".", file, maxsize) # Replace middle dot with dot
            file = re.sub(";", "؛", file, maxsize) # Replace English semicolon with Arabic semicolon
            file = re.sub("[“”]", '"', file, maxsize) # Replace two characers with quotation mark
            file = re.sub("…", "...", file, maxsize)
            file = re.sub("››", "«", file, maxsize) # Replace a character
            file = re.sub("‹‹", "»", file, maxsize) # Replace a character
            file = re.sub("[‹›]", "", file, maxsize) # Remvoe some characters
            file = re.sub(r"\s*ص\s*:\s*[۰-۹]+ *", "", file, maxsize) # Remove number of pages (e.g., ص: ۲۳)
            file = re.sub("ى", "ی", file, maxsize, re.MULTILINE) # Replace character code 0x649 with 0x6cc
            file = re.sub("ي", "ی", file, maxsize, re.MULTILINE) # Replace character code 0x64a with 0x6cc
            file = re.sub("ك", "ک", file, maxsize) # Replace ARABIC LETTER KAF with ARABIC LETTER KEHEH
            file = re.sub("۟", "ْ", file, maxsize) # Replace some characters wit Arabic Sukun
            file = re.sub("\t", " ", file, maxsize) # Replace tab character with space character
            arabic_chars = "ۣۢۨٙ"
            arabic_chars += "۪۠ۧ"
            arabic_chars += "ؙۭ۫۬ۜ"
            file = re.sub(f"[{arabic_chars}]", "", file, maxsize) # Remove some Arabic characters
            file = re.sub("\([۰-۹]*\)", "", file, maxsize) # Remove references indeices (e.g., "(23)"")
            if hard_clean:
            ...
            if len(normalize_len_interval) == 2:
                ...
                last_index = 0
                while True:
                    if last_index > file_size: break
                    line_length = randint(normalize_len_interval[0], normalize_len_interval[1]+1)
                    while (last_index+line_length) < file_size and file[last_index+line_length] != " ":
                        line_length += 1
                    new_file += file[last_index: (last_index+line_length)] + "\n"
                    last_index += line_length
                file = new_file[:-1] # Remove additional new line character at the end of the file
